...
SET la.imps=tmp.imps, la.clis=tmp.clis, la.spend=tmp.spend`
	...
	if res, err = db.Exec(ins_log, myDay); err != nil { return err }
	if logID, err = res.LastInsertId(); err != nil { return err }

	for slotID, arr := range slots {
		var lpID int64
		if res, err = db.Exec(ins_pub, logID, slotID, arr[0], arr[1]); err != nil { return err }
		if lpID, err = res.LastInsertId(); err != nil { return err }
		lpIds[slotID] = lpID
	}

	for itemID, arr := range items {
		var laID int64
		if res, err = db.Exec(ins_adv, logID, itemID, arr[0], arr[1]); err != nil { return err }
		if laID, err = res.LastInsertId(); err != nil { return err }
		laIds[itemID] = laID
	}
	...
	for slotID, lpID := range lpIds {
		for itemID, laID := range laIds {
			...
			if _, err = db.Exec(ins_pub_adv, lpID, laID, i, c, s); err != nil { return err }
			is += i
			cs += c
			ss += s
		}
	...
	if _, err = db.Exec(upd_pub, logID); err != nil { return err }
	if _, err = db.Exec(upd_adv, logID); err != nil { return err }
	if _, err = db.Exec(upd_log, is, cs, ss, logID); err != nil { return err }
...
WHERE ldp.daily=? AND lda.daily=?`
	...
	if _, err := db.Exec(ins_log, myDay); err != nil { return err }
	if _, err := db.Exec(ins_pub, myDay); err != nil { return err }
	if _, err := db.Exec(ins_adv, myDay); err != nil { return err }
	if _, err := db.Exec(ins_pub_adv, myDay, myDay, myDay); err != nil { return err }
