...
Before any production bid rollout, run the dependency-free configuration
preflight from the exact service environment:
...
It applies bid-mode validation, rejects the checked-in local/recovery tracking
secrets and values shorter than 32 bytes, emits no secret material, and does not
connect to MySQL, Redis, NATS, or HTTP services. Ordinary `Config.Validate`
remains compatible with deterministic local/test fixtures; production release
evidence requires `ValidateProduction` through this command.
...
completed original lane set plus S06/P03/S05/O03/R03, the remaining A03
remediation horizon, demand-gated I02, and the bounded
`GOAL.md` review-fix contract; checks
the 95/0/6/61 schema inventory; and rejects attempts to test the removed
...
Disposable MySQL verification restores the current clean baseline, expects 96
tables, 6 routines, and 61 triggers, proves `auth_security_audit` update/delete fails,
...
disposable MySQL lifecycle restores the current 96 tables/6 routines/61 triggers and proves
