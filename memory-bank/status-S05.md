...
- Iteration 1 (2026-08-24): P2, resolved — the outbound policy denied selected
  ...
  pass.
- Iteration 1 (2026-08-24): P2, open — Genelet exports string-valued form
  markers as principal provenance and `Controller.Handle` accepts caller-set
  forwarded session/MFA headers. A direct Handle or Summer filter caller can
  manufacture those values without the opaque-session gate, so sensitive
  actors are not yet backed by an unforgeable, exact-action/resource
