...
The package is currently being made runnable from a clean local Docker harness:
MySQL is the active relational backend, Redis is the runtime cache backend, and
NATS is the local message bus for spread/log flows.
...
- Publishers (`pub`) expose sites, slots, publisher attributes, and traffic
  metadata.
- Matching code turns database state into Redis/spread cache structures such as
  `PubMap`, `RAdv`, audience maps, and creative maps.
- DSP runtime code reads request data, config, Redis cache entries, MaxMind
  lookup data, and logging paths to produce bid responses and win/loss records.
