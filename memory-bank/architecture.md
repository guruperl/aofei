...
4. The cache job reads MySQL through `dsp.Config`, builds `PubMap`, the derived
   ...
   first flushes its subscription, then on a fresh node best-effort compiles an
   atomic disk generation from MySQL while holding the same cache-mutation lease
   as publishers; Redis supplies only the lease and monotonic sequence, so an
   older Redis snapshot cannot replace a just-subscribed spread-only publish.
   Existing or newly received complete generations skip bootstrap, including
   after lease acquisition. The receiver stages every declared entry below
...
Bidder targets and expanded callback targets share one `internal/safehttp`
address boundary. URL validation rejects an answer set if any member is in a
reviewed IPv4/IPv6 special-purpose deny range, while the dial path resolves and
checks again before pinning the connection to an approved address. IPv4-mapped
IPv6 addresses are normalized to IPv4; IPv6 is eligible only inside the
...
Arbitrary network round trippers fail closed; explicitly marked socket-free
test doubles remain subject to URL and redirect validation. Redirects are
limited to ten, revalidated after injected hooks, and stripped of all headers
and URL credentials after an authority or scheme change. Custom URL guards are
additional restrictions, never replacements for the shared boundary.
...
history. `cmd/aofei-deploy` strictly validates those inputs, manager reload
