...
uas = append(uas, packed)
	...
	for _, b := range bs {
		if b < 0 { continue }
		if !MatchBrowser(uas, uint32(b)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaBrowser), uint32(b))
		}
	}
	for _, bv := range bvs {
		if bv < 0 { continue }
		if !MatchBVersion(uas, uint32(bv)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaBVersion), uint32(bv))
		}
	}
	for _, o := range os {
		if o < 0 { continue }
		if !MatchOs(uas, uint32(o)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaOs), uint32(o))
		}
	}
	for _, ov := range ovs {
		if ov < 0 { continue }
		if !MatchOVersion(uas, uint32(ov)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaOVersion), uint32(ov))
		}
	}
	for _, p := range ps {
		if p < 0 { continue }
		if !MatchPlatform(uas, uint32(p)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaPlatform), uint32(p))
		}
	}
	for _, d := range ds {
		if d < 0 { continue }
		if !MatchDevice(uas, uint32(d)) {
			t.Errorf("%v %v", pzutil.MapUint32(uas, UnpackPzUaDevice), uint32(d))
		}
	}
