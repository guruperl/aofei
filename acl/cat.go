...
// Has returns true if the CategoryAudience contains the given CATSs
func (self *CategoryAudience) Has(white, black, pub, site, page []string) bool {
	...
	cats := append(page, append(site, pub...)...)
	if len(cats) > 0 {
		c := NewCATSFromStrings(cats)
		if self.Order == 1 && !self.Others.WhiteContain(c) {
			return false
		} else if self.Order == 0 && !self.Others.BlackContain(c) {
			return false
		}
	}
