...
func (self sfcap) Refresh(ctx context.Context, conn radix.Client, when time.Time, pid string, item *Item, act int) error {
	imp := self.Imp
	cli := self.Cli
	switch act {
	case FCAPImp:
	...
	object := sfcap{Imp: imp, Cli: cli}
	err := binary.Write(buf, binary.LittleEndian, object)
	if err != nil {
		return err
	}

	return conn.Do(ctx, radix.Cmd(nil, "HSET", fmt.Sprintf("fcap:%s:%d", pid, item.ItemId, buf.Bytes())))
}

func NewSFcaps(ctx context.Context, conn radix.Client, pid string, itemIDs []uint32) (map[uint32]sfcap, error) {
	names := []string{fmt.Sprintf("fcap:%s" + pid)}
	for _, itemID := range itemIDs {
		names = append(names, fmt.Sprintf("%s", itemID))
	}
...
func GetCapped(when time.Time, sfcaps map[uint32]sfcap, caps map[uint32]Cap) map[uint32]bool {
