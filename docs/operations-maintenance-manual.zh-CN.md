...
授权状态，并输出确定性的打包 `site_token`/`slot_token` 清单。出现任何错误都
不能发布该世代。卖方资料改动会撤销已有授权；运营人员须复核卖方 ID、类型、
ASI、公开名称和域名后再授权，不能把透明度资料当成结算指令。
...
使用 Codex slash goal 连续执行多个里程碑时，还必须读取根目录
`GOAL.md`，按调用方给出的 `STATUS_ORDER` 执行，并在每个里程碑关闭后
校正受影响的后续状态文件和剩余依赖顺序。
