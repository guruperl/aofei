...
原有 D/P/R/I/S/A/O 基线截至 A02 已完成；D04、D05、S06 与 P03 已完成，当前评审
整改从 S05 继续到 O03、R03、A03。P03 的威胁契约、默认关闭的 v2 定位符读取器及
SDK/服务端请求认证已完成；生产启用仍须另行完成具名发布商灰度。I02 原生
...
创建专用 `aofei` 用户和组：配置只读、二进制可执行、上传/spread/日志目录可写，代码仓库不可写。`nats-client` 会以 `0750` 创建缺失的日志目录；已有目录必须预先限制在不宽于 `0750`，程序只校验而不执行 `chmod`，从而保留 setgid/sticky 策略。日志文件为 `0640`；不要把账务输入设为全局可读或可写。
