...
| IFA, buyer id, user id, deprecated device ids | OpenRTB/SDK body | Approved local audience matching and frequency control | Raw value may be used transiently only in personalized local matching; unauthenticated SDK compatibility traffic is forced contextual even when it supplies an otherwise valid grant. HMAC domain-separated before cap keys or tracking payloads; never sent to middleman or audits | Request memory; pseudonymous cap state default 90 idle days |
| Exact geo and demographics | OpenRTB/SDK body or IP lookup | Approved local audience matching | An authenticated publisher/App may assert body IP/coarse geo only under a separate personalization grant; the assertion is not verified device location. Latitude, longitude, accuracy, and fix age are removed in every mode; other detailed geo and demographics are removed in contextual/restricted mode and from audits/bidder fanout | Request memory only |
...
| Public account human-verification data | Registration/recovery browser, direct peer, and Turnstile response | Prevent automated account creation and account-email abuse | Cloudflare's widget receives its security telemetry under the operator's Cloudflare contract; W8M sends the one-time token, derived client IP, and fixed action to Siteverify, retains no token, stores only expiring deployment-keyed HMAC email/IP quota keys in Redis, and records the resolved address on a successfully created account | Token is request-memory only; quota windows are at most 24 hours; the account registration address follows the account/security-record lifecycle; Cloudflare retention is an external processor-contract/notice gate before enablement |
| Publisher/App request credential and proof | S02-authorized publisher/operator issuance plus SDK/server request headers | Authenticate one approved App integration at `POST /pz` | MySQL stores the Ed25519 public verifier and scoped lifecycle metadata; the private seed is shown once and retained only by the publisher. Runtime keeps an immutable public-key snapshot and a short-lived Redis hash of public id plus nonce; raw private seed, signature, nonce, and request body are excluded from MySQL, Redis, audits, logs, and samples | Credential metadata through expiry/rotation/revocation and account policy; replay hash only through the accepted timestamp window; immutable lifecycle audit follows S02 retention |
...
3. For an uploaded audience, use `uploaded.DeleteAudienceIdentifier` for one
   ...
   Markers are normalized to `buyeruid`, `userid`, `ip`, `ifa`, `did`, `dpid`,
   or `mac`; the bounded `buyerid`/`user` legacy aliases are deleted together.
